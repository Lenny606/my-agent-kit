        ...
        if has_gsap:
            ...
            if not has_gsap_cleanup:
                if filename.endswith('.astro'):
                    self.warnings.append(f"[Motion] {filename}: GSAP animation without cleanup (kill/revert). Memory leak risk on unmount (less critical in static Astro pages).")
                else:
